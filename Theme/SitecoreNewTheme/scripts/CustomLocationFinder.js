/* eslint-disable */
XA.component.customlocationfinder = (function ($, document) {
  ...
  /**
	    ...
	    /** @lends module:locationfilter.LocationFilterView.prototype **/
	{
        ...
        updateHash: function (params, properties) {
            ...
            for (i = 0; i < sig.length; i++) {
                XA.component.search.vent.trigger("my-location-coordinates-changed", {
                    sig: sig[i],
                    coordinates: params[sig[i] !== "" ? sig[i] + "_cg" : "cg"].split("|")
                });
            }
        },
		/**
        ...
        */
        createHashObject: function(g, o, a) {
            ...
            for (i = 0; i < sig.length; i++) {
                signature = sig[i];
                hashObj[signature !== "" ? signature + "_cg" : "cg"] = g;
    ...
    */
    api.init = function() {
        ...
        var components = $(".custom-location-filter:not(.initialized)");
        ...
        _.each(components, function(elem) {
            ...
            if (!scriptsLoaded && properties.mode !== "Location") {
                mapsConnector.loadScript(properties.key, XA.component.customlocationfinder.scriptsLoaded);
            } else {
                initialize();
            }
...
XA.register("customlocationfinder", XA.component.customlocationfinder);