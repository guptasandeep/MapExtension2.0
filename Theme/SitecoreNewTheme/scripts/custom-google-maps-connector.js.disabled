...
XA.connector.mapsConnector = (function ($, document) {
        ...
        if (typeof infoWindows[key] !== "undefined") {
            (function(currentKey, marker) {
                google.maps.event.addListener(marker, "click", function() {
					...
					raiseInfoWindowOpenEvent(infoWindows[currentKey]); //Trigger an event upon InfoWindow is opened
                });
            })(key, marker);
        }
    };
        ...
        google.maps.event.addListener(marker, "click", function () {
            if (typeof (getPoiContent) === "function") {
                ...
                getPoiContent(poiId, poiTypeId, poiVariantId, function (result) {
					...
					raiseInfoWindowOpenEvent(infoWindows[mapId]); //Trigger an event upon InfoWindow is opened
                });
            }
        });
    };
    ...
    api.getLastMap = function() {
		return map;
    };
	
	function raiseInfoWindowOpenEvent(currentInfoWindow)
	{
		google.maps.event.addListenerOnce(currentInfoWindow, 'domready', function() {
			$(document).trigger('infowindowOpen', currentInfoWindow);
		});	
	}
